...
async function loadPlatform(platform = "qoder") {
  ...
  if (platform === "kimi") {
    const module = await import("./session-analysis/platforms/kimi.mjs");
    return {
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (options.help === true) {
    ...
    process.stdout.write([
      `Usage: session-analysis${command ? ` ${command}` : " <command>"} --platform <qoder|codex|claude|cursor|qwen|copilot|pi|kimi> --workspace <path> [options]`,
      "",
